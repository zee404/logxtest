...
var App = function () {
    ...
    return {
        ...
        side_Menu: function (id){

            //highlight menu link
            var close_li = $("#"+id).parent().addClass('open');

            //parent ul
            var parent_ul = $("#"+id).parent().parent();
            //parent li
            var parent_li = $("#"+id).parent().parent().parent();

            parent_ul.css('display', 'block');
            parent_li.addClass('open');

            //make parent arrow open
            parent_li.children('a').children('.arrow').addClass('open');

            //parent_li.children('.sub-menu').slideUp(200);

            var g_parent_ul = parent_li.parent().css('display', 'block');
            g_parent_ul.parent().addClass('open');

            //make main menu link arrow open
            g_parent_ul.parent().children('a').children('.arrow').addClass('open');
        },

        deliveries_status: function (status){
            var status_html = '';
            switch(status){
                case 'Not Assign':
                ...
                case 'Cancel':
                    ...
                    break;
                case 'Collected':
                    status_html = '<span class="label label-sm label-success">Collected</span>';
                    break;
                    case 'Completed':
                    status_html = '<span class="label label-sm label-success">Completed</span>';
                    break;
                default:
        ...
        bag_collection_status: function (status){
            var status_html = '';
            switch(status){
                case 'Assign':
                    status_html = '<span class="label label-sm label-warning">Assigned</span>';
                ...
                case 'Picked':
                    ...
                    break;
                case 'No bag':
                    status_html = '<span class="label label-sm label-danger">No bag</span>';
                    break;
                default:
                    status_html = '<span class="label label-sm label-default "></span>';
                    break;
            }
            return status_html;
        },

        get_customer_status: function(status){
            var status_html = '';
            switch(status){
                case 0:
