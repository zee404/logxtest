...
class Customer_model extends CI_Model{
    ...
    public function get_where($where){
        //$where['u.rold_id'] = 2;
        $query = $this->db->select('id, email, phone, full_name, address, code, vendor_id')->get_where($this->table, $where);
        return $query->result();
    }
    ...
    if(!empty($check_number) and count($check_number)>1){
       $this->db->select('*')
        ...
        if ($query->num_rows() > 0) {
            ...
            "address"=>$myresults["address"],
                                   "error"=>4,
                                    "error_detail"=>"Phone and Address doesnot matched.");                  
            return $returnResults;
        }else{
            $this->db->select('*')
                ->from('users')
 ...
 public function fashion_newss_blog($limit, $page)
    ...
    private function _get_datatables_query($postData){
        $this->db->select('u.id,u.full_name, u.phone,v.email,u.address');
        $this->db->from($this->table.' as u');
        $this->db->join($this->table.' as v', 'v.id = u.vendor_id'); 
        $i = 0;
