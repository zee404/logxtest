...
class Vehicle_status extends CI_Model{
    ...
    public function get_vehicle_status_availability($vehicle_id)
    {
        ...
        $result = $this->db->select('id,status,vehicle_id,driver_id')
            ->from('vehicle_status')
    ...
    public function get_vehicle_basic_($vid){
        
        $res=$this->db->select('id,model_name,type,qr_code')->from('vehicle')->where('id',$vid)->get()->result();
        
        return $res;
    }
    
     public function get_driver_vehicle_status_availability($driver_id)
    {
     ...
     public function check_out_driver_and_vehicle($driver_id,$data){
         $this->db->where_in('driver_id', $driver_id);
