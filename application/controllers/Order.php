...
//echo json_encode($deliveries);
    ...
    public function createqr(){
        ...
        $brand_code = $this->session->userdata('brand_code');
        $lastcode =$this->bag_model->get_last_qrcode()[0]['code'];

        $lastcode = (int)substr($lastcode, 4);
        echo "<br>";
        ...
        if(isset($quantity)){
            ...
            for($i=0; $i<$quantity; $i++){
                ...
                $qrString = strtoupper($brand_code.sprintf('%04u', $lastcode+1));    
    ...
    public function change_brand_code(){
        $this->load->model('Customer_model');
        $user_id = $this->session->userdata('u_id');
        $brand_code = strtoupper($_POST["brand_code"]);
        if (!$this->customer_model->check_brand_code_exist($brand_code))
        {
            ...
            $result = $this->customer_model->update($data, $where);
            if($result){
                $response['success'] = true;
                $session_data = array(
                    'brand_code' => $brand_code,
            ...
            echo json_encode (array("success"=>false,"error"=>"Brand code already exists."));
        }
    }
...
//  	 }
   ...
   public function tt(){
    ...
    //    $cdate=$this->uri->segment(3)? $this->uri->segment(3): date('Y-m-d', strtotime(date('Y-m-d'). ' + 1 days'));
    //     $next=$this->uri->segment(4)? $this->uri->segment(4): date('Y-m-d', strtotime(date('Y-m-d'). ' + 1 days'));
        $cdate = $this->uri->segment(3) ? $this->uri->segment(3) : date('Y-m-d');
        $next = $this->uri->segment(4) ? $this->uri->segment(4) : date('Y-m-d', strtotime($cdate . ' + 1 days'));
     ...
     public function assign_drivers_bagpicks_partner_wise()
    {
        ...
        if(count($vendor_ids) > 0){
            for($it=0;$it<count($vendor_ids);$it++){
             ...
             $where2 = "o.vendor_id=".$vendor_ids[$it]." and o.action != 'Freezed' and  o.action != 'Paused'  and (o.status = 'Not Assign' OR o.status = 'Assign') and o.bag_pickup_status='unassigned'and o.is_cancelled='no' and o.delivery_date BETWEEN '". date('Y-m-d', strtotime($cdate))." 00:00:00' and '". date('Y-m-d', strtotime($end_date))." 23:59:59'";
              if($emirate_id){
               $where2 = $where2." and o.emirateID = ".$emirate_id;
            }
...
//   if($this->session->userdata('role') == 'admin'){
        ...
        public function pending_orders()
    {
        $cdate = $this->uri->segment(3) ? $this->uri->segment(3) : date('Y-m-d');
        $next = $this->uri->segment(4) ? $this->uri->segment(4) : date('Y-m-d', strtotime($cdate . ' + 1 days'));
        $slot_id = $this->uri->segment(5) ? $this->uri->segment(5): 0;
        ...
        //     $where = "o.driver_id > 0 and o.status !='Delivered' and o.status!='Collected' and delivery_date BETWEEN '".$cdate." 00:00:00' and '".$next." 23:59:59'";
        if ($this->session->userdata('role') != 'vendor') {
            $where = "o.action != 'Freezed' and o.action != 'Paused' and o.driver_id > 0 and o.status !='Delivered' and o.status!='Collected'and slotID =".$slot_id."  and  delivery_date BETWEEN '" . $cdate . " 00:00:00' and '" . $next . " 23:59:59'";
        } else {
            $this->load->view('forbidden');
         }
        $data['orders'] =  $this->order_model->get_orders($where);
