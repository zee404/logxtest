...
// }
 ...
 public function unassigned_by_created_date()
    ...
    public function total_bags()
    {
        ...
        if(strtolower($this->session->userdata('role')) == 'vendor'){
            $vendor_id = $this->session->userdata('u_id');
            $where = "b.is_cancelled='no' and b.vendor_id=".$vendor_id;
        }else{
            $where = "b.is_cancelled='no'";
        }
        ...
        $data['orders'] =  $this->bag_model->get_where($where);
        $this->load->view('bag/total_bags', $data);
   ...
   public function bag_status_AC(){
    ...
    public function completed()
    {
       ...
       if(strtolower($this->session->userdata('role')) == 'vendor'){
            ...
            $where = "b.driver_id != 0 and b.status !='Assign' and b.status!='Requested'";
    ...
    public function total_by_created_date()
    {
   ...
   public function save_bag_form()
    {
      ...
      //  $send_notif['bag_notification']=$this->input->post('notification');
        ...
        // To get Emirate and slot id by name code start
         ...
         $ans=$this->bag_model->get_partner_price($vendor_id,$dt,$emirateID,$area_str,$service_typ,$string_emirate);
         //echo $this->db->last_query();
        // **************************** IF ANS HAS NO BAG WITH SAME LOCATION THEN ASSIGN BAG QUTATION PRICE AS PARTNER PRICE ELSE ASSIGN SAME LOCATION PRICE AS PARTNER PRICE
                            if($ans[0]->ans == 'no'){
...
// Taha minor changes according to live code
    ...
    public function change_payment_status(){
        ...
        $bagid =  (int)$this->input->post('bagid');
        ...
        $this->db->where('bag_id',$bagid);
        $this->db->update('bags',$data);
